...
/**
 * The company's audit ledger, as the server holds it (PRD §7.12, §8.1).
 *
 * Distinct from the Activity screen, which is the phone's own write history
 * for the pack it is linked to. This is every change on every pack, by
 * everyone, including changes issued remotely that never passed through this
 * phone at all.
 *
...
export function fromRow(row: LedgerRow): LedgerEvent {
  return {
    id: row.id,
...
export const DEFAULT_LEDGER_ROWS = 200;

/**
 * Filtering happens **server-side**. Fetching the newest 200 events and
 * narrowing them here would silently miss everything older — a pack whose
 * changes fall outside that window would report "nothing has been changed",
 * which is a false statement about an audit trail rather than an empty one.
 */
export async function listLedger(api: ApiClient, filter: LedgerFilter = {}): Promise<LedgerEvent[]> {
  const query: string[] = [];
  if (filter.batteryId) query.push(`batteryId=${encodeURIComponent(filter.batteryId)}`);
  if (filter.source) query.push(`source=${filter.source}`);
  if (filter.result) query.push(`result=${filter.result}`);
  if (filter.limit) query.push(`limit=${filter.limit}`);
  const suffix = query.length ? `?${query.join('&')}` : '';
  return (await api.get<{ events: LedgerRow[] }>(`/audit${suffix}`)).events.map(fromRow);
}
...
export const RESULT_TONE: Record<WriteResult, 'good' | 'warn' | 'critical'> = {
  success: 'good',
  adjusted: 'warn',
  rejected: 'critical',
  timeout: 'critical',
...
/**
 * `occurredAt` is the device's clock and `recordedAt` is the server's. They
 * are kept apart on purpose — an event uploaded three hours late did not
 * happen three hours late — so where they disagree materially, say so rather
 * than silently picking one.
 */
export const LATE_UPLOAD_MS = 5 * 60 * 1000;

export function wasUploadedLate(event: LedgerEvent): boolean {
...
export function describeChange(event: LedgerEvent): string {
