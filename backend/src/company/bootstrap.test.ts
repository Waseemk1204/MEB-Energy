...
const users = () =>
  store.all<{ id: string; email: string; role: string; company_id: string }>('SELECT * FROM users');
const companies = () => store.all<{ id: string; name: string }>('SELECT * FROM companies');
...
describe('the company and its first administrator', () => {
  ...
  it('names the company from the environment', async () => {
    ...
    assert.equal(companies()[0]!.name, 'Northern Haulage');
  });

  it('falls back to the default name when none is configured', async () => {
    await bootstrapCompany(store, GOOD);
    assert.equal(companies()[0]!.name, DEFAULT_COMPANY_NAME);
  });
  ...
  it('joins an existing company rather than creating a second one', async () => {
    ...
    assert.equal(outcome.kind, 'created');
    assert.equal(companies().length, 1);
    assert.equal(users()[0]!.company_id, 'c-existing');
  ...
  it('stores the email lowercased, since login looks it up that way', async () => {
    ...
    assert.equal(users()[0]!.email, 'ops@mebenergy.example');
  });
});
...
describe('what it refuses to do', () => {
  ...
  it('refuses a short password rather than accepting one', async () => {
    await assert.rejects(
      () => bootstrapCompany(store, { ...GOOD, password: 'short' }),
      BootstrapError
    );
  ...
  it('refuses something that is not an email address', async () => {
    await assert.rejects(() => bootstrapCompany(store, { ...GOOD, email: 'admin' }), BootstrapError);
    assert.equal(users().length, 0);
  });
});
