...
const rowsFor = async (p: Principal, batteryId = ACME_BATTERY) =>
  await queryHistory(store, p, { batteryId, limit: 5000 });
...
describe('downsampling', () => {
  it('keeps roughly one row per interval from a 2 Hz stream', async () => {
    ...
    const result = await ingestSamples(store, ACME, ACME_BATTERY, stream(120));
    assert.equal(result.received, 120);
    assert.equal(result.stored, 6);
  });

  it('discards the overwhelming majority of a live stream', async () => {
    const result = await ingestSamples(store, ACME, ACME_BATTERY, stream(1200)); // 10 minutes
  ...
  it('stores the first sample unconditionally', async () => {
    const result = await ingestSamples(store, ACME, ACME_BATTERY, [sample()]);
    assert.equal(result.stored, 1);
  });

  it('continues the cadence across separate batches', async () => {
    await ingestSamples(store, ACME, ACME_BATTERY, stream(20)); // 10 s
  ...
  it('handles a batch that arrives out of order', async () => {
    const shuffled = [...stream(40)].reverse();
    const result = await ingestSamples(store, ACME, ACME_BATTERY, shuffled);
    const rows = (await rowsFor(acme)).map((r) => r.recorded_at);
...
describe('fault transitions are never downsampled away', () => {
  it('keeps a sample that raises a fault mid-interval', async () => {
    // A fault appears one second in — well inside the 10 s window.
    const samples = stream(20, (i) => (i === 2 ? { faultCount: 1 } : {}));
    const result = await ingestSamples(store, ACME, ACME_BATTERY, samples);
    assert.ok(result.keptForStateChange >= 1);
    assert.ok((await rowsFor(acme)).some((r) => r.fault_count === 1));
  });

  it('keeps the sample that clears a fault too', async () => {
    const samples = stream(20, (i) => (i >= 2 && i <= 4 ? { faultCount: 1 } : {}));
    await ingestSamples(store, ACME, ACME_BATTERY, samples);
    const faults = (await rowsFor(acme)).map((r) => r.fault_count);
    assert.ok(faults.includes(1), 'the fault appearing');
    assert.ok(faults.includes(0), 'the fault clearing');
  });

  it('records the exact moment the fault appeared', async () => {
    const samples = stream(20, (i) => (i === 6 ? { faultCount: 2 } : {}));
    await ingestSamples(store, ACME, ACME_BATTERY, samples);
    const row = (await rowsFor(acme)).find((r) => r.fault_count === 2);
    assert.equal(row?.recorded_at, T0 + 6 * 500);
  });

  it('does not keep every frame of a sustained fault', async () => {
    // Faulted throughout: the state never changes, so normal cadence applies.
    const result = await ingestSamples(store, ACME, ACME_BATTERY, stream(120, () => ({ faultCount: 1 })));
...
describe('querying history', () => {
  beforeEach(async () => {
    await ingestSamples(store, ACME, ACME_BATTERY, stream(240)); // 120 s
    await ingestSamples(store, RIVAL, RIVAL_BATTERY, stream(240));
  });
  ...
  it('filters by time range', async () => {
    const rows = await queryHistory(store, acme, {
  ...
  it('caps an absurd limit rather than trusting it', async () => {
    await assert.doesNotReject(async () => await queryHistory(store, acme, { batteryId: ACME_BATTERY, limit: 10_000_000 }));
  });

  it('shows a tenant nothing of another’s battery', async () => {
    assert.equal((await rowsFor(rival, ACME_BATTERY)).length, 0);
  });

  it('shows even an administrator nothing of another company’s battery', async () => {
    assert.equal((await rowsFor(admin, RIVAL_BATTERY)).length, 0);
    assert.ok((await rowsFor(admin, ACME_BATTERY)).length > 0);
  });
});

/** Unlike the audit ledger, telemetry is expected to age out. */
describe('retention', () => {
  it('drops readings past the window', async () => {
    const old = Date.now() - RETENTION_MS - 60_000;
    await ingestSamples(store, ACME, ACME_BATTERY, [sample({ recordedAt: old })]);
    await ingestSamples(store, ACME, ACME_BATTERY, [sample({ recordedAt: Date.now() })]);
    assert.equal((await rowsFor(acme)).length, 2);

    await pruneTelemetry(store);
    assert.equal((await rowsFor(acme)).length, 1);
  });

  it('keeps readings inside the window', async () => {
    await ingestSamples(store, ACME, ACME_BATTERY, [
