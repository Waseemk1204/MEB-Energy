...
const seedUser = async (id: string, email: string, status = 'invited', hash = NO_PASSWORD) => {
  await store.run(
...
beforeEach(async () => {
  ...
  await seedCompany(store, 'c1', 'Acme', NOW);
  await store.run(
    'INSERT INTO users (id, company_id, email, display_name, role, password_hash, status, created_at) VALUES (?,?,?,?,?,?,?,?)',
    'admin-1', 'c1', 'ops@acme.example', 'Ops', 'company', 'hash', 'active', NOW
  );
  await seedUser('u-invited', 'new@acme.example');
});

afterEach(() => store.close());

describe('issuing an invitation', () => {
  it('returns a token the caller can hand over', async () => {
    const invitation = await createInvitation(store, 'u-invited', 'admin-1', NOW);
  ...
  it('never stores the token itself', async () => {
    const { token } = await createInvitation(store, 'u-invited', 'admin-1', NOW);
    const rows = await store.all<{ token_hash: string }>('SELECT token_hash FROM user_invitations');
  ...
  it('issues a different token every time', () => {
    const seen = new Set(
      Array.from({ length: 25 }, async () => (await createInvitation(store, 'u-invited', 'admin-1', NOW)).token)
    );
    assert.equal(seen.size, 25);
  });

  it('records who issued it', async () => {
    await createInvitation(store, 'u-invited', 'admin-1', NOW);
    const row = await store.get<{ invited_by: string }>('SELECT invited_by FROM user_invitations');
...
describe('accepting an invitation', () => {
  it('sets the password and activates the account', async () => {
    ...
    const row = await store.get<{ status: string; password_hash: string }>(
  ...
  it('replaces the placeholder that made the account unusable', async () => {
    ...
    const row = await store.get<{ password_hash: string }>(
  ...
  it('marks the invitation as spent', async () => {
    const { token } = await createInvitation(store, 'u-invited', 'admin-1', NOW);
    await acceptInvitation(store, token, GOOD_PASSWORD, NOW + 1000);
    const row = await store.get<{ accepted_at: number | null }>(
...
describe('what an invitation refuses', () => {
  ...
  it('cannot be used twice', async () => {
    ...
    await assert.rejects(
      async () => await acceptInvitation(store, token, 'a-different-passphrase', NOW + 2000),
  ...
  it('expires', async () => {
    const { token } = await createInvitation(store, 'u-invited', 'admin-1', NOW);
    await assert.rejects(
      async () => await acceptInvitation(store, token, GOOD_PASSWORD, NOW + INVITATION_TTL_MS + 1),
  ...
  it('works right up to the moment it expires', async () => {
    ...
    await assert.doesNotReject(async () =>
      await acceptInvitation(store, token, GOOD_PASSWORD, NOW + INVITATION_TTL_MS - 1)
    );
  });

  it('rejects a token nobody issued', async () => {
    await assert.rejects(
      async () => await acceptInvitation(store, 'not-a-real-token-at-all', GOOD_PASSWORD, NOW),
  ...
  it('rejects a password too short to be worth setting', async () => {
    const { token } = await createInvitation(store, 'u-invited', 'admin-1', NOW);
    await assert.rejects(
      async () => await acceptInvitation(store, token, 'x'.repeat(MIN_PASSWORD_LENGTH - 1), NOW + 1000),
  ...
  it('does not spend the invitation on a rejected password', async () => {
    ...
    await assert.doesNotReject(async () => await acceptInvitation(store, token, GOOD_PASSWORD, NOW + 2000));
  });
  ...
  it('does not activate the account when it refuses', async () => {
    ...
    const row = await store.get<{ status: string }>('SELECT status FROM users WHERE id = ?', 'u-invited');
    assert.equal(row?.status, 'invited');
  });
});

describe('what is still outstanding', () => {
  it('lists an unaccepted invitation without exposing its token', async () => {
    await createInvitation(store, 'u-invited', 'admin-1', NOW);
    const pending = await pendingInvitations(store, ['u-invited']);
  ...
  it('drops one that has been accepted', async () => {
    const { token } = await createInvitation(store, 'u-invited', 'admin-1', NOW);
    await acceptInvitation(store, token, GOOD_PASSWORD, NOW + 1000);
    assert.equal((await pendingInvitations(store, ['u-invited'])).size, 0);
  });

  it('reports the newest when an account was invited more than once', async () => {
    await createInvitation(store, 'u-invited', 'admin-1', NOW);
    await createInvitation(store, 'u-invited', 'admin-1', NOW + 5000);
    const entry = (await pendingInvitations(store, ['u-invited'])).get('u-invited');
    assert.equal(entry?.expiresAt, NOW + 5000 + INVITATION_TTL_MS);
  });

  it('is empty for a user who was never invited', async () => {
    await seedUser('u-direct', 'direct@acme.example', 'active', 'a-real-hash');
    assert.equal((await pendingInvitations(store, ['u-direct'])).size, 0);
  });

  it('handles being asked about nobody', async () => {
    assert.equal((await pendingInvitations(store, [])).size, 0);
  });
});
