...
/**
 * Assisted remote control — the console's half of Mode 1 (PRD §7.3, §7.10).
 *
 ...
 * queue, and the console must say so rather than showing a tick.
 *
 * That is not a limitation to be smoothed over in the UI — it is the safety
 * property the whole architecture is built on. An administrator who believes a
 * change has landed when it has not is exactly the failure Mode 1 prevents.
 */
...
export async function listSessions(api: ApiClient): Promise<SupportSession[]> {
  const body = await api.get<{ sessions: SessionRow[] }>('/support-sessions');
  return body.sessions.map(fromRow);
}

export async function openSession(api: ApiClient, batteryId: string): Promise<string> {
  const body = await api.post<{ supportSessionId: string }>('/support-sessions', { batteryId });
...
export async function closeSession(
  ...
  const body = await api.patch<{ cancelledCommands: number }>(`/support-sessions/${sessionId}`, {
    outcome,
  });
...
export async function issueCommand(
  ...
  return api.post<IssueResult>(`/support-sessions/${sessionId}/commands`, input);
}
...
export async function parametersFor(api: ApiClient, bmsModel: string): Promise<Parameter[]> {
  const body = await api.get<{ parameters: Parameter[] }>(
...
export function describeDisposition(disposition: Disposition): { text: string; tone: 'good' | 'warn' } {
  return disposition === 'deliverable'
    ...
    : {
        text: 'Queued. Nobody is linked to this battery, so it will wait until someone is.',
        tone: 'warn',
      };
}
 ...
 * support convenience. The console says so at the point of use.
 */
export const FORCE_PUSH_CAVEAT =
  'Force Push moves this ahead of other queued changes. It does not deliver it any sooner: a technician still has to be linked to the battery.';
